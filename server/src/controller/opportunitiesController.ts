...
export const opportunitiesController = new Elysia({ prefix: '/api' }).get('/opportunities', async ({ query }) => {
  ...
  const candidates = rankOpportunityCandidates(prices, filters).slice(0, 10)
  ...
  const items = enriched.filter(item => (item.dailyVolume ?? 0) >= filters.minVolume).slice(0, filters.limit)
  const value = { generatedAt: new Date().toISOString(), partial, filters, items }
  cache.set(cacheKey, { expires: Date.now() + 5 * 60 * 1000, value })
  return value
})
