...
export default function Opportunities({ locale }: { locale: 'th' | 'en' }) {
  ...
  return <main className="container mx-auto max-w-5xl px-4 py-6 sm:py-9" lang={locale}>
    ...
    {!query.isFetching && !query.isError && query.data?.items.length === 0 && <div className="state-card mt-5"><TrendingUp className="mx-auto h-7 w-7 text-primary" /><h2>{th ? 'ยังไม่พบโอกาสจากข้อมูลช่วงนี้' : 'No opportunities in this time range'}</h2><p>{th ? 'คุณไม่ได้กรอกผิด แต่อาจยังไม่มีผู้เล่นส่งราคา Asia ที่สดพอ ลองดูข้อมูลเก่าขึ้นและตรวจราคาในเกมอีกครั้ง' : 'Your filters are valid, but recent Asia data may be limited. Try older data and verify prices in game.'}</p>{filters.maxAgeMinutes !== 1440 ? <button type="button" onClick={includeOlderData} className="nav-link nav-link-primary mt-4">{th ? 'ลองดูข้อมูลไม่เกิน 24 ชั่วโมง' : 'Try data up to 24 hours old'}</button> : <button type="button" onClick={reset} className="nav-link mt-4">{th ? 'กลับไปใช้ค่าตั้งต้น' : 'Reset filters'}</button>}</div>}
    {!!query.data?.items.length && <section className="mt-5 grid gap-4 lg:grid-cols-2" aria-live="polite">{query.data.items.map(item => {
      ...
      return <article key={`${item.itemId}-${item.sourceCity}-${item.targetCity}`} className="market-item opportunity-card">
        <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">{item.itemName}</h2><p className="font-mono text-xs text-muted-foreground">{item.itemId}</p></div><span className={`confidence-badge ${tone}`}>{item.confidence} confidence</span></div>
